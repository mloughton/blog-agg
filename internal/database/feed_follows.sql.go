...
const getFeedFollowsAll = `-- name: GetFeedFollowsAll :many
SELECT id, feed_id, user_id, created_at, updated_at FROM feed_follows
`

func (q *Queries) GetFeedFollowsAll(ctx context.Context) ([]FeedFollow, error) {
	rows, err := q.db.QueryContext(ctx, getFeedFollowsAll)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
